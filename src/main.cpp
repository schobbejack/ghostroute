...
namespace ghostroute
{
	...
	static void reply_icmp(const SendMessage auto &sock, const message &msg, const in6_addr &target, icmp_type type, Code code)
	{
		...
		const auto payload_length = std::min(std::size(data_buffer) - icmp_header::size, static_cast<size_t>(msg.length) - 14);
		...
		std::span payload_source(reinterpret_cast<std::byte *>(msg.io.iov_base) + 14, std::size(payload));
